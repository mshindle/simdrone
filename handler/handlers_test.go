...
func MakeTestServer(dispatcher bus.Dispatcher) *negroni.Negroni {
	server := negroni.New() // don't need all the middleware here or logging.
	mx := mux.NewRouter()
	initRoutes(mx, dispatcher)
...
func TestAddValidTelemetryCreatesCommand(t *testing.T) {
	...
	dispatcher := dispatch.NewMockDispatcher()
	...
	var telemetryResponse bus.TelemetryUpdatedEvent
...
func TestAddValidPositionCreatesCommand(t *testing.T) {
	...
	reader := bytes.NewReader(body)
	request, _ = http.NewRequest("POST", "/api/cmds/position", reader)
	server.ServeHTTP(recorder, request)
	...
	var positionResponse bus.PositionChangedEvent
	payload := recorder.Body.Bytes()
...
func TestAddValidAlertCreatesCommand(t *testing.T) {
	...
	reader := bytes.NewReader(body)
	request, _ = http.NewRequest("POST", "/api/cmds/alert", reader)
	server.ServeHTTP(recorder, request)
	...
	var alertResponse bus.AlertSignalledEvent
	payload := recorder.Body.Bytes()
